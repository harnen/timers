...
Forwarder::onOutgoingInterest(const shared_ptr<pit::Entry>& pitEntry, Face& outFace, const Interest& interest)
{
  ...
  auto pathInterest = make_shared<Interest>(interest);
  if(!interest.getRepeated()){
	  NFD_LOG_DEBUG("Before " << interest);
	  NS_LOG_DEBUG("Hop count " << hopCount);
	  pathInterest->setPath(hopCount, outFace.getId() - 255);
	  //pathInterest->addPath(outFace.getId() - 255);
	  NFD_LOG_DEBUG("Added " << outFace.getId() << " to the path");
	  NFD_LOG_DEBUG("After " << *pathInterest);
  // insert out-record
  }
  pitEntry->insertOrUpdateOutRecord(outFace, *pathInterest);

  // send Interest
  outFace.sendInterest(*pathInterest);
...
Forwarder::onIncomingData(Face& inFace, const Data& data)
{
  ...
  for (Face* pendingDownstream : pendingDownstreams) {
	  NS_LOG_DEBUG("pendingDownStream");
...
Forwarder::setUnsatisfyTimer(const shared_ptr<pit::Entry>& pitEntry, int deadline)
{
  ...
  NFD_LOG_DEBUG("scheduling unsatisfyTimer in (ns)" << lastExpiryFromNow + time::milliseconds(deadline));
  pitEntry->m_unsatisfyTimer = scheduler::schedule(lastExpiryFromNow + time::milliseconds(deadline),
