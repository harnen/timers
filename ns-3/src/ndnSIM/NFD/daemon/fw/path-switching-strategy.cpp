...
#include "path-switching-strategy.hpp"
...
NFD_LOG_INIT("PathSwitchingStrategy");

const Name PathSwitchingStrategy::STRATEGY_NAME("ndn:/localhost/nfd/strategy/path-switching/%FD%04");
NFD_REGISTER_STRATEGY(PathSwitchingStrategy);

const time::milliseconds PathSwitchingStrategy::RETX_SUPPRESSION_INITIAL(10);
const time::milliseconds PathSwitchingStrategy::RETX_SUPPRESSION_MAX(250);

PathSwitchingStrategy::PathSwitchingStrategy(Forwarder& forwarder, const Name& name)
...
void
PathSwitchingStrategy::afterReceiveInterest(const Face& inFace, const Interest& interest,
                                         const shared_ptr<pit::Entry>& pitEntry)
{
  ...
  unsigned int pathId = (unsigned int) interest.getPath(hopCount);
  NS_LOG_DEBUG("pathId: " <<  pathId);
  Name func_prefix("/exec");
  if(func_prefix.isPrefixOf(interest.getName()) && pathId){
	  Face* facePath = getFace((FaceId) (pathId + 255));
	  NS_LOG_DEBUG("Forwarding to face from the path: " <<  facePath->getId());
	    this->sendInterest(pitEntry, *facePath, interest);
	    return;
  }
...
void
PathSwitchingStrategy::afterReceiveNack(const Face& inFace, const lp::Nack& nack,
                                     const shared_ptr<pit::Entry>& pitEntry)
{
