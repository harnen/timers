...
NS_LOG_COMPONENT_DEFINE("ndn.ConsumerApp");
...
{
  static TypeId tid =
    TypeId("ns3::ndn::ConsumerApp")
      .SetGroupName("Ndn")
      .SetParent<App>()
      .AddAttribute("StartSeq", "Initial sequence number", IntegerValue(0),
                    MakeIntegerAccessor(&ConsumerApp::m_seq), MakeIntegerChecker<int32_t>())
	  ...
	  .AddAttribute("AppDelay",
					...
					UintegerValue(2000),
					MakeUintegerAccessor(&ConsumerApp::m_appDelay),
					MakeUintegerChecker<uint32_t>())

      .AddTraceSource("LastRetransmittedInterestDataDelay",
                      "Delay between last retransmitted Interest and received Data",
                      MakeTraceSourceAccessor(&ConsumerApp::m_lastRetransmittedInterestDataDelay),
                      "ns3::ndn::ConsumerApp::LastRetransmittedInterestDataDelayCallback")

      .AddTraceSource("FirstInterestDataDelay",
                      "Delay between first transmitted Interest and received Data",
                      MakeTraceSourceAccessor(&ConsumerApp::m_firstInterestDataDelay),
                      "ns3::ndn::ConsumerApp::FirstInterestDataDelayCallback");
...
void
ConsumerApp::SetRetxTimer(Time retxTimer)
{
  ...
  m_retxEvent = Simulator::Schedule(m_retxTimer, &ConsumerApp::CheckRetxTimeout, this);
}

Time
ConsumerApp::GetRetxTimer() const
{
  return m_retxTimer;
}

void
ConsumerApp::CheckRetxTimeout()
{
...
// Application Methods
void
ConsumerApp::StartApplication() // Called at time specified by Start
{
  NS_LOG_FUNCTION_NOARGS();
...
void
ConsumerApp::StopApplication() // Called at time specified by Stop
{
  NS_LOG_FUNCTION_NOARGS();
...
void
ConsumerApp::SendPacket(Name name, uint32_t seq){
  if (!m_active){
...
void
ConsumerApp::OnData(shared_ptr<const Data> data)
{
  if (!m_active)
    return;
...
void
ConsumerApp::OnNack(shared_ptr<const lp::Nack> nack)
{
  /// tracing inside
  App::OnNack(nack);
...
void
ConsumerApp::OnTimeout(uint32_t sequenceNumber)
{
...
void
ConsumerApp::WillSendOutInterest(uint32_t sequenceNumber)
{
