...
public:
	...
	wireEncode(EncodingImpl<TAG>& encoder,
			bool wantUnsignedPortionOnly = false) const;
	...
	std::shared_ptr<std::array<uint8_t, PATH_SIZE>> getPath() const{
		   auto newPath = std::make_shared<std::array<uint8_t, PATH_SIZE>>();
		   for(int i = 0; i < PATH_SIZE; i++){
			   (*newPath)[i] = m_path[i];
		   }
		   return newPath;
	...
	Data& setPath(int index, uint8_t val) {
		m_path[index] = val;
		return *this;
	}

	Data& setPath(std::shared_ptr<std::array<uint8_t, PATH_SIZE>> array) {
		for(int i = 0; i < PATH_SIZE; i++){
			m_path[i] = array->at(i);
		}
	...
	unsigned int getRepeated() const {
			return m_repeated;
		}
...
inline bool Data::hasWire() const {
...
inline uint32_t Data::getContentType() const {
