...
Interest::Interest() :
		m_interestLifetime(time::milliseconds::min()), m_selectedDelegationIndex(
				INVALID_SELECTED_DELEGATION_INDEX) {
	memset(m_path, (int) 0, PATH_SIZE);
}

Interest::Interest(const Name& name) :
		m_name(name), m_interestLifetime(time::milliseconds::min()), m_selectedDelegationIndex(
				INVALID_SELECTED_DELEGATION_INDEX) {
	memset(m_path, (int) 0, PATH_SIZE);
}

Interest::Interest(const Name& name, const time::milliseconds& interestLifetime) :
		m_name(name), m_interestLifetime(interestLifetime), m_selectedDelegationIndex(
				INVALID_SELECTED_DELEGATION_INDEX) {
	memset(m_path, (int) 0, PATH_SIZE);
}
...
Interest::setNonce(uint32_t nonce) {
	if (m_wire.hasWire() && m_nonce.value_size() == sizeof(uint32_t)) {
		std::memcpy(const_cast<uint8_t*>(m_nonce.value()), &nonce,
				sizeof(nonce));
	} else {
...
bool Interest::matchesName(const Name& name) const {
	...
	// name must include implicit digest
			!(name.size() - m_name.size()
					<= static_cast<size_t>(getMaxSuffixComponents())))
		return false;
...
bool Interest::matchesData(const Data& data) const {
	...
	size_t minSuffixComponents =
			hasMinSuffixComponents ?
	...
	if (!getExclude().empty() && fullNameLength > interestNameLength) {
		if (interestNameLength == fullNameLength - 1) {
			...
			if (getExclude().isExcluded(
					data.getFullName().get(interestNameLength)))
				return false;
	...
	const KeyLocator& publisherPublicKeyLocator =
			this->getPublisherPublicKeyLocator();
...
size_t Interest::wireEncode(EncodingImpl<TAG>& encoder) const {
	...
	if (hasLink()) {
		if (hasSelectedDelegation()) {
			totalLength += prependNonNegativeIntegerBlock(encoder,
					tlv::SelectedDelegation, m_selectedDelegationIndex);
		}
	...
	if (getInterestLifetime() >= time::milliseconds::zero()
		...
		totalLength += prependNonNegativeIntegerBlock(encoder,
				tlv::InterestLifetime, getInterestLifetime().count());
	}
...
void Interest::wireDecode(const Block& wire) {
	...
	if (m_wire.type() != tlv::Interest)
		BOOST_THROW_EXCEPTION(
				Error("Unexpected TLV number when decoding Interest"));
	...
	if (val != m_wire.elements_end()) {
		...
		if (selectedDelegation
				< uint64_t(Link::countDelegationsFromWire(m_link))) {
...
void Interest::setLink(const Block& link) {
	m_link = link;
	if (!link.hasWire()) {
		BOOST_THROW_EXCEPTION(
				Error("The given link does not have a wire format"));
	}
	m_wire.reset();
	m_linkCached.reset();
...
bool Interest::hasSelectedDelegation() const {
	return m_selectedDelegationIndex != INVALID_SELECTED_DELEGATION_INDEX;
}

Name Interest::getSelectedDelegation() const {
	if (!hasSelectedDelegation()) {
		BOOST_THROW_EXCEPTION(
				Error("There is no encapsulated selected delegation"));
	}
	return std::get<1>(
			Link::getDelegationFromWire(m_link, m_selectedDelegationIndex));
}

void Interest::setSelectedDelegation(const Name& delegationName) {
	size_t delegationIndex = Link::findDelegationFromWire(m_link,
			delegationName);
	if (delegationIndex != INVALID_SELECTED_DELEGATION_INDEX) {
		m_selectedDelegationIndex = delegationIndex;
	} else {
		BOOST_THROW_EXCEPTION(
				std::invalid_argument("Invalid selected delegation name"));
	}
	m_wire.reset();
}

void Interest::setSelectedDelegation(size_t delegationIndex) {
...
void Interest::unsetSelectedDelegation() {
	m_selectedDelegationIndex = INVALID_SELECTED_DELEGATION_INDEX;
	m_wire.reset();
}
...
operator<<(std::ostream& os, const Interest& interest) {
	...
	if (interest.getMinSuffixComponents() >= 0) {
		os << delim << "ndn.MinSuffixComponents="
				<< interest.getMinSuffixComponents();
		delim = '&';
	}
	if (interest.getMaxSuffixComponents() >= 0) {
		os << delim << "ndn.MaxSuffixComponents="
