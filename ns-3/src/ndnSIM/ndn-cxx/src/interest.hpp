...
public:
	...
	Interest& setPath(int index, uint8_t val) {
		m_path[index] = val;
		return *this;
	}

	Interest& setPath(uint8_t* array) {
		memcpy(m_path, array, PATH_SIZE);
		return *this;
	}

	Interest& setPath(std::shared_ptr<std::array<uint8_t, PATH_SIZE>> array) {
		for (int i = 0; i < PATH_SIZE; i++) {
			m_path[i] = array->at(i);
		}
...
inline std::string Interest::toUri() const {
